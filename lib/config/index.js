...
const Config = Schema.object({
  ...
  重复指令惩罚: Schema.object({
    启用: Schema.boolean().default(true).description("是否启用重复错误的惩罚机制"),
    初始罚款: Schema.number().default(100).description("首次罚款额度"),
    倍率: Schema.number().default(2).description("每次继续试错后的加倍倍率"),
    追踪窗口: Schema.number().default(5 * 60 * 1e3).description("在该时间内重复视为连续尝试（毫秒）"),
    忽略指令: Schema.array(Schema.string()).default(["抢劫"]).description("不触发惩罚的指令名称（按首个词匹配）")
  }).default({
    启用: true,
    初始罚款: 100,
    倍率: 2,
