...
const Config = Schema.object({
  ...
  初始身价: Schema.number().default(1e3),
  初始身价: Schema.number().default(1e3),
  ...
  打工收入范围: Schema.object({
    最低: Schema.number().default(60),
    最高: Schema.number().default(80)
  }).description("打工基础收入区间"),
  牛马主加成: Schema.number().default(0.05),
  打工税率: Schema.number().default(0.05).description("打工所得税率"),
  // 冷却时间(毫秒)
  ...
  天灾概率: Schema.number().default(0.25).description("农场收获时触发天灾事件的概率"),
  // 概率配置
  抢劫成功率: Schema.number().default(0.3),
  抢劫策略: Schema.object({
    名称: Schema.string().default("标准"),
    描述: Schema.string().default("统一的抢劫策略"),
    成功率: Schema.number().default(0.4),
    抢夺比例: Schema.number().default(0.3),
    惩罚比例: Schema.number().default(0.3)
  }).description("抢劫策略配置"),
  ...
  购买税率: Schema.number().default(0.1).description("所有消费额外征收的税率"),
  金融手续费: Schema.object({
    比例: Schema.number().default(0.1).description("金融操作手续费百分比"),
    最低: Schema.number().default(10).description("手续费最低金额")
  }).description("金融操作手续费设置"),
  // 决斗配置
  ...
  监狱系统: Schema.object({
    ...
    最低打工收入: Schema.number().default(20),
    最高打工收入: Schema.number().default(40),
    手续费比例: Schema.number().default(0.3),
    手续费下限: Schema.number().default(5),
    打工冷却: Schema.number().default(10 * 60 * 1e3).description("每次监狱打工的最小冷却时间"),
    抢劫入狱概率: Schema.number().default(0.5).description("抢劫失败后被关进监狱的概率"),
    越狱成功率: Schema.number().default(0.1).description("越狱成功的概率"),
    越狱失败惩罚: Schema.number().default(10).description("越狱失败增加的刑期（分钟）")
  }),
  // 管理员列表配置
  ...
  贷款系统: Schema.object({
    ...
    手续费: Schema.number().default(100).description("每次贷款需要额外支付的固定费用")
  ...
  排行榜: Schema.object({
    排除管理员: Schema.boolean().default(false).description("是否在排行榜展示时排除管理员")
  }).default({
    排除管理员: false
  }).description("排行榜展示相关配置"),
  群控制: Schema.object({
    白名单: Schema.array(Schema.string()).default([]).description("只允许这些群/频道使用指令，留空表示不限制"),
    黑名单: Schema.array(Schema.string()).default([]).description("禁止这些群/频道使用指令")
  }).default({
    白名单: [],
    黑名单: []
  }).description("群组访问控制"),
  用户控制: Schema.object({
    白名单: Schema.array(Schema.string()).default([]).description("只允许这些用户使用指令，留空表示不限制"),
    黑名单: Schema.array(Schema.string()).default([]).description("禁止这些用户使用指令")
  }).default({
    白名单: [],
    黑名单: []
  }).description("用户访问控制")
