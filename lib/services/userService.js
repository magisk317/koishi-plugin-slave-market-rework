...
const { isResetBanned } = require("../utils/resetLimit");
...
function createUserService(options) {
  ...
  async function ensureNotBanned(ctx, session) {
    if (!session?.userId)
      return null;
    return await isResetBanned(ctx, session.userId) ? "🚫 由于多次重开违规，你已被永久封禁，无法再使用该功能。" : null;
  }
  ...
  async function ensurePlayerProfile(ctx, config, session, options = {}) {
    ...
    const banMessage = await ensureNotBanned(ctx, session);
    if (banMessage) return { created: false, bonus: 0, user: null };
  ...
  async function getUser(ctx, userId, session) {
    const banned = await ensureNotBanned(ctx, session);
