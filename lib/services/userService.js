...
function createUserService(options) {
  ...
  async function ensurePlayerProfile(ctx, config, session, options = {}) {
    const targetUserId = options.targetUserId || session?.userId;
    if (!targetUserId) return { created: false, bonus: 0, user: null };
    const scopeId = getScopeKey(session);
    const scopedUserId = buildScopedId(scopeId, targetUserId);
    ...
    const cardName = options.nickname || resolveGroupCard(session);
    if (existing.length) {
      const updates = {};
