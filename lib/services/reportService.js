...
function createReportService(ctx, options = {}) {
  ...
  async function collectStatistics(config) {
    const [users, statsRows, systemRows] = await Promise.all([
...
📈 总交易次数：${formatNumber(data.stats.totalTransactions || 0)}
💼 总打工收入：${formatNumber(data.stats.totalWorkIncome || 0)}
🪙 总抢劫金额：${formatNumber(data.stats.totalRobAmount || 0)}
