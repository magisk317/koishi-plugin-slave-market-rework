...
const { createWebUI } = require("./modules/webui");
...
function calculateLoanLimit(user, config) {
  ...
  // Removed loanCreditLevel usage
  return Math.floor(loanConfig.基础额度);
}
...
var inject = {
  required: ["database"],
  optional: ["recall", "puppeteer", "server"],
  weather: WeatherService
};
async function apply(ctx, config) {
  ctx.logger.info('[slave-market] Plugin apply function started');
  state.setRuntimeConfig(config);
  registerModels(ctx);
  ctx.logger.info('[slave-market] Models registered');
  ...
  // Initialize WebUI
  createWebUI(ctx, config, { plant, crops, harvest });
  ...
  ctx.command('大牛马时代.webui', '获取WebUI登录令牌')
    .action(async ({ session }) => {
      ...
      return `WebUI 登录令牌: ${token}\n有效期 30 分钟。请在网页端输入此令牌登录。`;
    });
  ...
  ctx.logger.info('[slave-market] About to register commands');
  registerAllCommands(ctx, config);
  ctx.logger.info('[slave-market] Commands registered');
...
async function applyDailyAssetDecay(ctx) {
  ...
  // Run VIP card cleanup
  try {
    await cleanupExpiredVipCards(ctx);
  } catch (error) {
    ctx.logger?.warn?.("[slave-market] failed to cleanup expired vip cards", error);
  }
}
