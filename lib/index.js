...
__name(shouldEnableRecall, "shouldEnableRecall");
...
function resolveNickname(session) {
    ...
    return pickString(session.member?.name, session.member?.nick, (_a = session.event?.member) === null || _a === void 0 ? void 0 : _a.name, (_b = session.event?.member) === null || _b === void 0 ? void 0 : _b.nick, (_c = session.author) === null || _c === void 0 ? void 0 : _c.name, (_d = session.author) === null || _d === void 0 ? void 0 : _d.nick, (_f = (_e = session.onebot) === null || _e === void 0 ? void 0 : _e.sender) === null || _f === void 0 ? void 0 : _f.card, (_h = (_g = session.onebot) === null || _g === void 0 ? void 0 : _g.sender) === null || _h === void 0 ? void 0 : _h.nickname, session.user?.nickname, (_j = session.user) === null || _j === void 0 ? void 0 : _j.name, session.username, (_k = session.author?.user) === null || _k === void 0 ? void 0 : _k.nickname);
}
...
function normalizeGender(input) {
    ...
    if (!value)
        return null;
    if (["男", "male", "man", "m", "1", "♂", "boy"].includes(value))
        return "男";
    if (["女", "female", "woman", "f", "0", "♀", "girl"].includes(value))
        return "女";
    return null;
}
__name(normalizeGender, "normalizeGender");
function resolveGender(session) {
    var _a, _b, _c, _d, _e, _f, _g;
    const candidates = [
        session.author?.user?.gender,
        session.author?.gender,
        session.user?.gender,
        session.member?.user?.gender,
        (_a = session.event?.member) === null || _a === void 0 ? void 0 : _a.user?.gender,
        session.onebot?.sender?.sex,
        session.onebot?.user?.sex,
        session.onebot?.user?.gender,
        (_c = (_b = session.onebot) === null || _b === void 0 ? void 0 : _b.sender) === null || _c === void 0 ? void 0 : _c.gender,
        (_e = (_d = session.onebot) === null || _d === void 0 ? void 0 : _d.info) === null || _e === void 0 ? void 0 : _e.sex,
        (_g = (_f = session.event) === null || _f === void 0 ? void 0 : _f.user) === null || _g === void 0 ? void 0 : _g.gender
    ];
...
__name(resolveGender, "resolveGender");
function registrationGuide() {
    return "❌ 你还没有注册！\n💡 发送\"注册玩家\"即可自动完成注册。";
}
__name(registrationGuide, "registrationGuide");
function registrationShortGuide() {
    return '请先发送"注册玩家"完成注册';
}
__name(registrationShortGuide, "registrationShortGuide");
const RECALL_METHODS = [
    { method: "send", cacheKey: "__slaveMarketOriginalSend" },
    { method: "sendQueued", cacheKey: "__slaveMarketOriginalSendQueued" },
];
function restoreRecallMethods(session) {
    for (const { method, cacheKey } of RECALL_METHODS) {
        if (session[cacheKey]) {
            session[method] = session[cacheKey];
            session[cacheKey] = void 0;
        }
    }
}
__name(restoreRecallMethods, "restoreRecallMethods");
function applyRecallMethods(session, ctx, config, actionKey) {
    for (const { method, cacheKey } of RECALL_METHODS) {
        const current = typeof session[method] === "function" ? session[method].bind(session) : null;
        if (!current && !session[cacheKey])
            continue;
        session[cacheKey] ??= current;
        if (!session[cacheKey])
            continue;
        session[method] = createRecallSender(session, ctx, config, actionKey, session[cacheKey]);
    }
}
__name(applyRecallMethods, "applyRecallMethods");
function scheduleMessageRecall(ctx, session, config, actionKey, sendResult) {
  ...
  const delay = Number.isFinite(delaySeconds) ? Math.max(0, delaySeconds) * 1e3 : 6e4;
  const ids = Array.isArray(sendResult) ? sendResult : [sendResult];
  for (const id of ids) {
    if (!id)
      continue;
    ctx.logger.info("[slave-market][recall] schedule delete", {
      actionKey,
      messageId: id,
      delay
    });
    setTimeout(() => {
      session.bot?.deleteMessage(session.channelId, id).then(() => {
        ctx.logger.info("[slave-market][recall] delete success", { actionKey, messageId: id });
      }).catch((error) => {
        ctx.logger.warn(`撤回 slave-market 消息失败: ${error.message}`, {
          actionKey,
          messageId: id
        });
      });
...
__name(scheduleMessageRecall, "scheduleMessageRecall");
async function invokeSendWithRecall(session, ctx, config, actionKey, sendFn, args) {
  if (!shouldEnableRecall(config, actionKey))
    return sendFn(...args);
  ...
  scheduleMessageRecall(ctx, session, config, actionKey, result);
  return result;
}
__name(invokeSendWithRecall, "invokeSendWithRecall");
...
__name(createRecallSender, "createRecallSender");
function setupMessageRecall(session, ctx, config, actionKey) {
    const recallConfig = config?.messageRecall;
    if (!shouldEnableRecall(recallConfig, actionKey)) {
        ctx.logger.info("[slave-market][recall] setup skipped", {
            actionKey,
            enabled: false
        });
        restoreRecallMethods(session);
        return;
    }
    applyRecallMethods(session, ctx, config, actionKey);
    ctx.logger.info("[slave-market][recall] setup applied", {
        actionKey,
        enabled: true
    });
}
__name(setupMessageRecall, "setupMessageRecall");
...
async function apply(ctx, config) {
  ...
  slaveCommand.subcommand("注册玩家 [nickname:string] [gender:string]", "注册成为玩家").action(async ({ session }, nickname, gender) => {
    setupMessageRecall(session, ctx, config, "general");
    const users = await ctx.database.get("player_market_users", { userId: session.userId });
    if (users.length) {
      return "❌ 你已经注册过了";
    }
    const finalNickname = nickname?.trim() || resolveNickname(session) || `玩家${session.userId}`;
    const manualGender = normalizeGender(gender);
    const profileGender = resolveGender(session);
    if (!manualGender && !profileGender) {
      return `⚠️ 无法读取到你的资料性别，请使用"注册玩家 昵称 性别"手动注册（性别仅支持男/女）。`;
    }
    const finalGender = manualGender ?? profileGender ?? "男";
    const now = Date.now();
    const userData = {
