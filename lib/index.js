...
function scheduleMessageRecall(ctx, session, config, actionKey, sendResult) {
  ...
  logDebug(ctx, "scheduleMessageRecall", {
    actionKey,
    delay,
    messageIds: ids
  });
  for (const id of ids) {
    ...
    setTimeout(() => {
      ...
      }).catch((error) => ctx.logger.warn(`撤回 slave-market 消息失败: ${error.message}`, {
        actionKey,
        messageId: id
      }));
    }, delay);
...
async function invokeSendWithRecall(session, ctx, recallConfig, actionKey, sendFn, args) {
  ...
  logDebug(ctx, "invokeSendWithRecall", {
    actionKey,
    hasResult: Boolean(result)
  });
...
__name(sendWithRecall, "sendWithRecall");
function setupMessageRecall(session, ctx, config, actionKey) {
  ...
  if (!shouldEnableRecall(recallConfig, actionKey)) {
    ...
    logDebug(ctx, "setupMessageRecall skipped", {
      actionKey
    });
    if (recallSession.__slaveMarketOriginalSend) {
      session.send = recallSession.__slaveMarketOriginalSend;
      recallSession.__slaveMarketOriginalSend = void 0;
    }
    if (recallSession.__slaveMarketOriginalSendQueued) {
      session.sendQueued = recallSession.__slaveMarketOriginalSendQueued;
      recallSession.__slaveMarketOriginalSendQueued = void 0;
    }
    return;
  }
  if (!recallSession.__slaveMarketOriginalSend) {
    recallSession.__slaveMarketOriginalSend = session.send.bind(session);
  }
  if (!recallSession.__slaveMarketOriginalSendQueued && typeof session.sendQueued === "function") {
    recallSession.__slaveMarketOriginalSendQueued = session.sendQueued.bind(session);
  }
  ctx.logger.info("[slave-market][recall] setup applied", {
    actionKey,
    enabled: true
  });
  logDebug(ctx, "setupMessageRecall applied", {
    actionKey
  });
  session.send = createRecallSender(session, ctx, config, actionKey, recallSession.__slaveMarketOriginalSend);
  if (typeof recallSession.__slaveMarketOriginalSendQueued === "function") {
    session.sendQueued = createRecallSender(session, ctx, config, actionKey, recallSession.__slaveMarketOriginalSendQueued);
  }
}
...
async function bankInfo(ctx, config, session) {
  ...
  const loanRate = (getLoanConfig(config).利率 ?? 0.02) * 100;
  return `=== 银行账户信息 ===
...
async function applyLoan(ctx, config, session, amount) {
  let user = await getUser3(ctx, session.userId, session);
  if (typeof user === "string") {
    return user;
  }
  ...
  return `✅ 贷款成功！本次贷款${amount}金币
当前贷款余额：${currentLoan + amount}
剩余额度：${limit - (currentLoan + amount)}`;
}
__name(applyLoan, "applyLoan");
async function repayLoan(ctx, config, session, amount) {
  let user = await getUser3(ctx, session.userId, session);
  if (typeof user === "string") {
    return user;
  }
...
async function transfer(ctx, config, session, target, amount) {
  ...
  if (privileged) {
    ...
    return `✅ 管理员特权：已无损转账${amount}金币给${receiver.nickname}`;
  }
...
function allocateRedPacketAmount(packet) {
  ...
  const min = 1;
  const max = packet.remainingAmount - (packet.remainingShares - 1) * min;
...
📷 请扫描下方赞赏码完成赞助`;
  ...
  slaveCommand.subcommand("生成vip卡 <cardType:string> <count:number> [duration:string]", "生成指定类型的VIP卡密").action(async ({ session }, cardType, count, duration) => {
    setupMessageRecall(session, ctx, config, "general");
    if (!isAdmin(ctx, config, session.userId, session)) {
      return "只有管理员可以生成VIP卡密";
    }
    const typeInfo = normalizeVipCardType(cardType);
    if (!typeInfo) {
      return "❌ 无效的VIP卡类型，可用：日卡、周卡、月卡、小时卡";
    }
    ...
    if (typeInfo.key === "hour" && !duration) {
      return "❌ 小时卡需要提供有效时长，例如\"2\"或\"1-3\"";
    }
    const generator = createVipDurationGenerator(typeInfo, duration);
    if (!generator) {
      return "❌ 无效的小时卡时长，支持1-23小时并可使用起止范围";
    }
    ...
    return `✅ 已生成${list.length}张${typeInfo.label}
${list.join("\n")}`;
  });
...
👥 新牛马：${targetUser.nickname}`;
  ...
  slaveCommand.subcommand("抢劫 [target:string] [strategy:string]", "抢劫指定用户的余额（有失败风险）").action(async ({ session }, target, strategyArg) => {
    ...
    return rob(ctx, config, session, targetId, strategyName);
  });
  ...
  slaveCommand.subcommand("贷款 <amount:number>", "申请贷款，额度与信用等级挂钩").action(async ({ session }, amount) => {
    ...
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    return applyLoan(ctx, config, session, amount);
  });
  slaveCommand.subcommand("还款 <amount:number>", "偿还贷款并降低负债").action(async ({ session }, amount) => {
    setupMessageRecall(session, ctx, config, "general");
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    return repayLoan(ctx, config, session, amount);
  });
  ...
  slaveCommand.subcommand("发红包 <amount:number> <count:number>", "发放群红包，5%手续费").action(async ({ session }, amount, count) => {
    ...
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    return sendRedPacket(ctx, config, session, amount, count);
  });
  slaveCommand.subcommand("抢红包 <packetId:string>", "抢指定红包").action(async ({ session }, packetId) => {
    setupMessageRecall(session, ctx, config, "general");
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    return grabRedPacket(ctx, config, session, packetId);
  });
