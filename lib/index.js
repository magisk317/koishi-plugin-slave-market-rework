...
const { createPenaltyService } = require("./services/penaltyService");
...
const penaltyService = createPenaltyService({
  getUser2,
  logDebug
});
setResponseHook(async ({ session, ctx, config, actionKey, content, sendExtra }) => {
  await penaltyService.handleResponse({
    session,
    ctx,
    config,
    actionKey,
