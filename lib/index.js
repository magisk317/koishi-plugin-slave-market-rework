...
var Config = import_koishi2.Schema.object({
  ...
  奴隶主加成: import_koishi2.Schema.number().default(0.1),
  // 冷却时间(毫秒)
  ...
  // 奴隶福利配置
  奴隶福利: import_koishi2.Schema.object({
...
async function work(ctx, config, session) {
  ...
  const employerShare = user.employer ? Math.floor(income * config.奴隶主加成) : 0;
...
async function apply(ctx, config) {
  ...
  const slaveCommand = ctx.command("大奴隶时代", "大奴隶时代游戏");
...
🔗 立即赞助成为VIP：
https://afdian.com/item/fd7899e40b2711f0a67d52540025c377
...
https://afdian.com/item/fd7899e40b2711f0a67d52540025c377`;
  ...
  slaveCommand.subcommand("奴隶市场", "查看所有可购买的玩家列表").action(async () => {
    const users = await ctx.database.get("player_market_users", {});
    return formatMarketList(users);
  });
  slaveCommand.subcommand("我的奴隶", "查看自己拥有的所有奴隶信息").action(async ({ session }) => {
...
👥 新奴隶：${targetUser.nickname}`;
  ...
  slaveCommand.subcommand("打工", "打工赚钱，奴隶主可获得额外收入").action(async ({ session }) => {
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
  ...
  slaveCommand.subcommand("放生 <target:string>", "无条件解除与指定奴隶的购买关系").action(async ({ session }, target) => {
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    const users = await ctx.database.get("player_market_users", {});
    const targetUser = users.find((user) => user.nickname === target);
  ...
  slaveCommand.subcommand("奴隶排行", "查看拥有奴隶数量最多的玩家排行").action(async ({ session }) => {
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    const users = await ctx.database.get("player_market_users", {});
    ...
    const list = sorted.map(
      (user, index) => `${index + 1}. ${user.nickname} - 拥有奴隶: ${user.employeeCount}个`
    ).join("\n");
    return `=== 奴隶拥有量排行榜(前20名) ===
${list}`;
  });
...
${adminList}`;
  ...
  slaveCommand.subcommand("奴隶列表", "查看自己的奴隶").action(async ({ session }) => {
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    const employees = await ctx.database.get("player_market_users", {
      employer: session.userId
  ...
  slaveCommand.subcommand("奴隶状态 <target:string>", "查看奴隶状态").action(async ({ session }, target) => {
    const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
    if (taxCheck) return taxCheck;
    const users = await ctx.database.get("player_market_users", {});
    const targetUser = users.find((user) => user.nickname === target);
...
💎 福利等级：${targetUser.welfareLevel}`;
  ...
  slaveCommand.subcommand("赞助", "查看赞助信息").action(async ({ session }) => {
    return `💝 感谢您对游戏的支持！
🔗 赞助链接：https://afdian.com/item/fd7899e40b2711f0a67d52540025c377
...
function formatMarketList(users) {
  ...
  if (!freeUsers.length) return "市场目前没有可购买的玩家";
...
function formatEmployeeList(employees) {
  if (!employees.length) return "你还没有奴隶";
