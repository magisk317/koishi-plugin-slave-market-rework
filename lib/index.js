...
function createRecallSender(session, ctx, config, actionKey, baseSend) {
  ...
  const serviceSender = maybeCreateServiceSender(session, ctx, recallConfig, actionKey);
  if (serviceSender)
    return (...args) => serviceSender(...args);
  if (!shouldEnableRecall(recallConfig, actionKey))
...
function setupMessageRecall(session, ctx, config, actionKey) {
  ...
  if (serviceSender) {
    ...
    session.send = serviceSender;
    ctx.logger.info("[slave-market][recall] setup applied (service)", {
      actionKey,
      enabled: true
    });
    return;
...
function maybeCreateServiceSender(session, ctx, recallConfig, actionKey) {
  if (!ctx.recall)
    return null;
  if (!shouldEnableRecall(recallConfig, actionKey))
    return null;
  const delaySeconds = Number(recallConfig?.delay ?? 60);
  const delay = Number.isFinite(delaySeconds) ? Math.max(0, delaySeconds) : 60;
  return ctx.recall.createSender(session, actionKey ?? "general", {
    enabled: true,
    delay,
    autoReply: false
  });
}
__name(maybeCreateServiceSender, "maybeCreateServiceSender");
...
async function ensureAdminVipPrivileges(ctx, session, user) {
  if (!user || !session || !runtimeConfig)
    return user;
  if (!isAdmin(ctx, runtimeConfig, user.userId, session))
    return user;
...
function isAdmin(ctx, config, userId, session) {
  const scopedId = ensureScopedId(session, userId);
  return config.管理员列表.includes(scopedId);
}
