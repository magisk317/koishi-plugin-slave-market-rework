...
function resolveProgressiveTaxRate(baseRate, depositAmount = 0, options = {}) {
  ...
  let progressiveRate = normalizedRate + tier * PROGRESSIVE_STEP;
  progressiveRate = Math.min(PROGRESSIVE_MAX_RATE, progressiveRate);
  if (options.wealthProtected) {
    progressiveRate *= 0.5;
  }
  return Math.max(0, progressiveRate);
}

function calculateProgressiveAmount(baseRate, amount, depositAmount, options = {}) {
  if (!amount || amount <= 0)
    return 0;
  const rate = resolveProgressiveTaxRate(baseRate, depositAmount, options);
  if (rate <= 0)
    return 0;
...
function calculatePurchaseTax(config, amount, user, options = {}) {
  if (!amount || amount <= 0)
    return 0;
  const rate = resolvePurchaseTaxRate(config);
  if (rate <= 0)
    return 0;
  const effectiveRate = resolveProgressiveTaxRate(rate, user?.deposit ?? 0, options);
  return Math.max(0, Math.floor(amount * effectiveRate));
}
...
function calculateFinancialFee(config, amount, user, options = {}) {
  if (!amount || amount <= 0) return 0;
  const { ratio, minimum } = resolveFinancialFeeConfig(config);
  const percentFee = calculateProgressiveAmount(ratio, amount, user?.deposit ?? 0, {
    minimum,
    wealthProtected: options.wealthProtected
  });
...
async function ensureSufficientBalance(ctx, user, requiredAmount, options = {}) {
  ...
  if (deposit < deficit) {
    const notice = `💡 当前余额 ${user.balance}，银行存款 ${deposit}，合计仍不足 ${requiredAmount}，银行资金暂未动用。`;
    return { user, withdrawn: 0, notice };
  }
  const withdrawAmount = deficit;
  if (withdrawAmount <= 0) {
    return { user, withdrawn: 0, notice: "" };
  }
  const updatedUser = {
