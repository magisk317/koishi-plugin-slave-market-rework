...
function createBankModule(deps) {
  ...
  async function applyBankPenalty(ctx, session, user, penaltyAmount, options = {}) {
    ...
    let remaining = Math.min(Math.max(0, penaltyAmount), totalBalance + totalDeposit);
    ...
    const applied = deductedBalance + deductedDeposit;
    ...
    if (applied > 0) {
      await transactionService?.logTransaction(ctx, { ...user, balance: newBalance, deposit: newDeposit }, {
        direction: "expense",
      ...
      await creditSystemAccount(ctx, applied);
    }
    invalidateWealthCache(session);
    return applied;
  }
  async function handleInvalidDepositInput(ctx, session, user) {
    const level = user.depositPenaltyLevel ?? 0;
    const penalty = 100 * Math.pow(2, level);
    const applied = await applyBankPenalty(ctx, session, user, penalty, {
      nextPenaltyLevel: level + 1,
      description: "非法存款罚金",
      reason: "invalid_deposit"
    });
    const shortfall = applied < penalty ? "（余额不足，已扣至身无分文）" : "";
    return `⚠️ 存款必须填写正整数，请不要恶意尝试！本次罚款${applied}金币${shortfall}，下次罚款翻倍。`;
  }
  async function handleOverdraftAttempt(ctx, session, user, targetAmount) {
    ...
    const applied = await applyBankPenalty(ctx, session, user, penalty, {
      ...
      description: "超额存款罚金",
      reason: "overdraft_deposit"
    });
    const shortfall = applied < penalty ? "（余额不足，已扣至身无分文）" : "";
    return `⚠️ 存款金额【${targetAmount}】超出可用余额，属于违规操作！本次罚款${applied}金币${shortfall}，罚款金额将继续翻倍。`;
  }
  ...
  async function deposit(ctx, config, session, rawAmount) {
    ...
    const fee = privileged ? 0 : Math.min(amount, calculateFinancialFee(config, amount, user, { wealthProtected }));
