...
function createBankModule(deps) {
  ...
  async function deposit(ctx, config, session, rawAmount) {
    ...
    if ((!amount || amount <= 0) && !amountProvided) {
    ...
    const availableBalance = Math.max(0, user.balance ?? 0);
    const usableAmount = privileged ? amount : Math.min(amount, availableBalance);
    let fee = privileged ? 0 : Math.min(usableAmount, calculateFinancialFee(config, usableAmount, user, { wealthProtected }));
    let waiverTip = "";
    if (!privileged && fee > 0) {
    ...
    const actualDeposit = Math.max(0, usableAmount - fee);
    ...
    const updatedBalance = Math.floor(user.balance - usableAmount);
