...
function createMarketModule(deps) {
  ...
  function resolveRobStrategy(config) {
    ...
    if (!strategies.length) return defaultStrategy;
    const normalized = strategies.map((item) => ({
      名称: item.名称 ?? defaultStrategy.名称,
      描述: item.描述 ?? defaultStrategy.描述,
      成功率: clampRatio(item.成功率, defaultStrategy.成功率),
      抢夺比例: clampRatio(item.抢夺比例, defaultStrategy.抢夺比例),
      惩罚比例: clampRatio(item.惩罚比例, defaultStrategy.惩罚比例),
    }));
    ...
    const aggregated = normalized.reduce(
      (acc, current) => {
        acc.成功率 += current.成功率;
        acc.抢夺比例 += current.抢夺比例;
        acc.惩罚比例 = Math.max(acc.惩罚比例, current.惩罚比例);
        return acc;
      },
      { 成功率: 0, 抢夺比例: 0, 惩罚比例: 0 },
    );
    ...
    return {
      名称: "综合策略",
      描述: "整合原有策略，兼具收益与风险",
      成功率: clampRatio(aggregated.成功率 / count, defaultStrategy.成功率),
      抢夺比例: clampRatio(aggregated.抢夺比例 / count, defaultStrategy.抢夺比例),
      惩罚比例: clampRatio(
        aggregated.惩罚比例 || defaultStrategy.惩罚比例,
        defaultStrategy.惩罚比例,
      ),
    };
  }
  ...
  async function trySendRobberToJail(ctx, config, robber) {
  ...
  async function robUser(ctx, config, session, targetId) {
    ...
    if (success) {
      ...
      const jailTip = await trySendRobberToJail(ctx, config, robber);
      const penaltyTip = penalty > actualPenalty ? `（实际扣除${actualPenalty}金币，已被扣至0余额）` : "";
      return `抢劫失败（${strategy.名称}）！损失了${actualPenalty}${penaltyTip}${jailTip ? `\n${jailTip}` : ""}`;
    }
  }
  ...
  async function redeem(ctx, config, session) {
    ...
    if (slave.balance < ransomAmount) {
      ...
      return `❌ 赎身失败：需要${ransomAmount}金币，但余额只有${slave.balance}金币${notice}`;
    }
...
👥 新牛马：${targetUser.nickname}${notice}`);
    ...
    slaveCommand.subcommand("抢牛马 <target:string>", "支付翻倍价格强制抢走别人的牛马").action(async ({ session }, targetInput) => {
      ...
      if (!privileged && employer.balance < forcedPrice) {
        const notice = autoWithdrawNotice ? `\n${autoWithdrawNotice}` : "";
        return await respond(`❌ 余额不足，需要${forcedPrice}金币${notice}`);
      }
