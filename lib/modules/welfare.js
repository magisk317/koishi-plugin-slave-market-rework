...
function createWelfareModule(deps) {
  const { setupMessageRecall, getUser6, registrationGuide, transactionService, shopEffects } = deps;
...
🎯 今日剩余尝试次数：${remainingAttempts}/${TRAINING_DAILY_LIMIT}`;
  ...
  async function trainUser(ctx, config, session) {
    ...
    if (dailyCount >= TRAINING_DAILY_LIMIT) {
      return `📅 今日训练次数已达上限（${TRAINING_DAILY_LIMIT}次），明天再继续努力吧。`;
    }
    const currentLevel = Math.max(1, user.trainingLevel || 1);
    ...
    await transactionService?.logTransaction(ctx, { ...user, balance: updatedBalance }, {
      ...
      amount: totalCost,
      description: `培训费用${succeeded ? "" : "（训练失败）"}（基础${baseCost}+动态${dynamicCost}）`,
