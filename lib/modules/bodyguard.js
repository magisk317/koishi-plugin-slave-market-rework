...
function createBodyguardModule(deps) {
  ...
  async function hireBodyguard(ctx, config, session, guardName) {
    ...
    if (!privileged) {
      const cover = await ensureSufficientBalance(ctx, user, guard.price, { privileged });
      user = cover.user;
      autoWithdrawNotice = cover.notice;
    }
    if (!privileged && user.balance < guard.price) {
      const notice = autoWithdrawNotice ? `\n${autoWithdrawNotice}` : "";
      return `❌ 余额不足，需要${guard.price}金币${notice}`;
    }
    const balanceAfter = privileged ? user.balance : user.balance - guard.price;
    ...
    if (!privileged) {
      await transactionService?.logTransaction(ctx, { ...user, balance: balanceAfter }, {
        direction: "expense",
        category: transactionService?.categories.BODYGUARD,
        amount: guard.price,
        description: `雇佣 ${guard.name}`,
        balanceAfter
      });
    }
    const notice = autoWithdrawNotice ? `\n${autoWithdrawNotice}` : "";
    return `✅ 雇佣成功！获得${guard.name}保护${guard.duration / (60 * 60 * 1e3)}小时
💰 花费：${formatCostTip(privileged, guard.price)}${notice}`;
  }
