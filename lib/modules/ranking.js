function createRankingModule(deps) {
  const { setupMessageRecall, checkTaxBeforeCommand, createScopeFilter, renderRankingBoard, isAdmin } = deps;
  ...
  function shouldExcludeAdmins(config) {
    return config?.排行榜?.排除管理员 === true;
  }

  async function filterUsers(ctx, session, config) {
    const users = await ctx.database.get("player_market_users", createScopeFilter(session));
    if (!shouldExcludeAdmins(config)) return users;
    return users.filter((user) => !isAdmin(ctx, config, user.userId, session));
  }
  ...
  function registerRankingCommands(ctx, config) {
    ...
    slaveCommand.subcommand("牛马排行", "查看拥有牛马数量最多的玩家排行").action(async ({ session }) => {
      const users = await filterUsers(ctx, session, config);
      const sorted = users
    ...
    slaveCommand.subcommand("身价排行", "查看身价最高的玩家排行").alias("身价榜").action(async ({ session }) => {
      const users = await filterUsers(ctx, session, config);
      const sorted = users
    ...
    slaveCommand.subcommand("资金排行", "查看总资产最多的玩家排行").alias("财富排行").alias("财富榜").action(async ({ session }) => {
      const users = await filterUsers(ctx, session, config);
      const sorted = users
