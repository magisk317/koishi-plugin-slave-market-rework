...
⏰ 到期时间：${new Date(expireTime).toLocaleString()}`;
  ...
  async function checkVipStatus(ctx, config, session, targetUserId) {
    let user;
    if (targetUserId) {
      user = await getUser(ctx, targetUserId, session);
      if (!user) {
        // Try resolving if not found directly (though getUser usually handles it if passed correct ID)
        // But getUser wraps getUser6/2 which might return string error or null.
        // Let's use resolveRegisteredUser logic but adapted for target
        const resolved = await resolveRegisteredUser(ctx, { ...session, userId: targetUserId }); // Hacky way to reuse if it depends on session.userId
        // Actually resolveRegisteredUser uses session.userId.
        // Let's just use getUser directly and handle null.
      }
    } else {
    ...
    if (!user && targetUserId) {
      // Try to resolve via helper if getUser failed (e.g. for mentions)
      // But command argument parsing usually handles this.
      // If we use <target:user>, we get the ID.
      user = await getUser(ctx, targetUserId, session);
    }
  ...
  async function removeVip(ctx, config, session, targetUserId) {
    if (!isAdmin(ctx, config, session.userId, session)) {
      return "❌ 只有管理员可以使用此命令";
    }
    if (!targetUserId) return "❌ 请指定要删除VIP的用户";

    const user = await getUser(ctx, targetUserId, session);
    if (!user) return "❌ 找不到该用户";
  ...
  function registerVipCommands(ctx, config) {
    ...
    slaveCommand.subcommand("vip状态 [target:user]", "查看VIP状态").action(async ({ session }, target) => {
      const respond = setupMessageRecall(session, ctx, config, "general");
...
${list.join("\n")}`);
    ...
    slaveCommand.subcommand("删除VIP <target:user>", "【管理员】删除指定用户的VIP状态").action(async ({ session }, target) => {
      const respond = setupMessageRecall(session, ctx, config, "general");
