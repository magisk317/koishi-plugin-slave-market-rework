...
const { creditSystemAccount, ensureSufficientBalance } = require("../utils/economy");
...
💡 使用"踩缝纫机"完成劳动或输入"交保出狱"立即离开监狱`;
  ...
  async function bailOut(ctx, config, session) {
    ...
    const cover = await ensureSufficientBalance(ctx, user, bailAmount);
    user = cover.user;
    autoWithdrawNotice = cover.notice;
    if (user.balance < bailAmount) {
      const notice = autoWithdrawNotice ? `\n${autoWithdrawNotice}` : "";
      return `❌ 交保需要${bailAmount}金币，但你只有${user.balance}金币${notice}`;
    }
    const now = Date.now();
    const balanceAfter = user.balance - bailAmount;
    await ctx.database.set("player_market_users", { userId: user.userId }, {
      balance: balanceAfter,
      isInJail: false,
      jailStartTime: 0,
    ...
    await creditSystemAccount(ctx, bailAmount);
