const { Random } = require("koishi");
const { creditSystemAccount, ensureSufficientBalance, calculatePurchaseTax } = require("../utils/economy");
const { isWealthProtected, invalidateWealthCache } = require("../utils/wealthProtection");

function createJailModule(deps) {
  const {
    setupMessageRecall,
    checkTaxBeforeCommand,
    getUser6,
    getScopeKey,
    transactionService,
    shopEffects,
    resolveTargetUser
  } = deps;
  const applyTaxWaiverHelper = shopEffects?.applyTaxWaiver
    ? shopEffects.applyTaxWaiver
    : async (ctx, session, user, fee) => ({ amount: fee, waived: false, tip: "" });

  function shuffleStrategies(strategies) {
    const copy = strategies.slice();
    for (let i = copy.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [copy[i], copy[j]] = [copy[j], copy[i]];
    }
    return copy;
  }

  function resolveJailCooldown(config) {
    const value = Number(config?.监狱系统?.打工冷却);
    if (!Number.isFinite(value)) {
      return 10 * 60 * 1e3;
    }
    return Math.max(0, value);
  }

  function getJailCooldownReference(user) {
    if (!user) return 0;
    const startTime = Math.max(0, user.jailStartTime || 0);
    const lastWorkTime = Math.max(0, user.lastJailWorkTime || 0);
    // Penalty is added to the effective start time, making it seem like work started later (or just requiring more time passed)
    // Actually, we should add penalty to the *required duration* or subtract from *elapsed time*.
    // Let's handle it in resolveJailCooldown or where it's used.
    return Math.max(startTime, lastWorkTime);
  }

  function calculateBailAmount(user, config) {
    const basePrice = Number.isFinite(user?.price) && user.price > 0
      ? user.price
      : Math.max(1, Number(config?.初始身价) || 1);
    return Math.max(1, Math.floor(basePrice / 2));
  }

  async function getScopeCandidates(ctx, session, excludeIds = []) {
    const scopeId = getScopeKey(session);
    const filter = scopeId ? { scopeId } : {};
    const users = await ctx.database.get("player_market_users", filter);
    return users.filter((user) => !excludeIds.includes(user.userId));
  }

  async function distributeJailIncome(ctx, session, prisoner, amount) {
    if (!amount || amount <= 0) return "";
    const strategies = shuffleStrategies([
      async () => {
        const candidates = await getScopeCandidates(ctx, session, [prisoner.userId]);
        if (!candidates.length) return null;
        const target = candidates.reduce((lowest, current) => {
          const lowestWealth = (lowest.balance || 0) + (lowest.deposit || 0);
          const currentWealth = (current.balance || 0) + (current.deposit || 0);
          return currentWealth < lowestWealth ? current : lowest;
        });
        const balanceAfter = target.balance + amount;
        await ctx.database.set("player_market_users", { userId: target.userId }, {
          balance: balanceAfter,
        });
        await transactionService?.logTransaction(ctx, { ...target, balance: balanceAfter }, {
          direction: "income",
          category: transactionService?.categories.JAIL_DISTRIBUTION,
          amount,
          description: "监狱补贴",
          balanceAfter
        });
        invalidateWealthCache(session);
        return `💝 【资金最少】把${amount}金币塞给最穷的 ${target.nickname}`;
      },
      async () => {
        const candidates = await getScopeCandidates(ctx, session, [prisoner.userId]);
        if (!candidates.length) return null;
        const recipient = candidates[Math.floor(Math.random() * candidates.length)];
        const balanceAfter = recipient.balance + amount;
        await ctx.database.set("player_market_users", { userId: recipient.userId }, {
          balance: balanceAfter,
        });
        await transactionService?.logTransaction(ctx, { ...recipient, balance: balanceAfter }, {
          direction: "income",
          category: transactionService?.categories.JAIL_DISTRIBUTION,
          amount,
          description: "随机红包",
          balanceAfter,
          relatedUserId: prisoner.userId
        });
        invalidateWealthCache(session);
        return `🧧 【群友红包】随机送${amount}金币给 ${recipient.nickname}`;
      },
    ]);
    for (const handler of strategies) {
      const result = await handler();
      if (result) return result;
    }
    await creditSystemAccount(ctx, amount);
    return `⚠️ 没有找到合适的受赠者，${amount}金币已上缴系统`;
  }

  async function jailWork(ctx, config, session) {
    const user = await getUser6(ctx, session.userId, session);
    if (typeof user === "string") return user;
    if (!user.isInJail) {
      return "🚔 你又没被关进来，想踩缝纫机先排队进监狱";
    }
    const cooldown = resolveJailCooldown(config);
    const now = Date.now();
    const workStartTime = user.jailWorkStartTime || 0;

    // Case 1: Not started yet
    if (!workStartTime) {
      await ctx.database.set("player_market_users", { userId: user.userId }, {
        jailWorkStartTime: now
      });
      const minutes = Math.ceil(cooldown / 60000);
      return `🔨 开始踩缝纫机！\n⏳ 需要持续劳动 ${minutes} 分钟才能完成任务。\n⚠️ 期间请勿越狱，完成后再次输入指令即可出狱。`;
    }

    // Case 2: Working, but not finished
    const penalty = user.jailPenalty || 0;
    const effectiveCooldown = cooldown + penalty;
    if (now - workStartTime < effectiveCooldown) {
      const remaining = Math.ceil((effectiveCooldown - (now - workStartTime)) / 60000);
      return `⏳ 正在踩缝纫机中...\n💦 还要坚持 ${remaining} 分钟才能完成任务。${penalty > 0 ? `\n⚠️ 含越狱失败惩罚 ${Math.ceil(penalty / 60000)} 分钟` : ""}`;
    }

    // Case 3: Finished
    const baseIncome = Random.int(100, 150);
    const multiplier = Number(config?.监狱系统?.工作收入倍率 ?? 1);
    const income = Math.max(0, Math.floor(baseIncome * multiplier));
    let payoutMessage = "";
    if (income > 0) {
      payoutMessage = await distributeJailIncome(ctx, session, user, income);
    }
    await ctx.database.set("player_market_users", { userId: user.userId }, {
      isInJail: false,
      jailStartTime: 0,
      jailWorkStartTime: 0,
      jailReason: "",
      jailWorkIncome: 0,
      jailWorkCount: 0,
      lastJailWorkTime: now,
      lastJailVictimId: "",
    });
    return `✅ 缝纫任务完成！
💰 本次踩缝纫机收入：${income}金币${payoutMessage ? `\n${payoutMessage}` : ""}
🏁 你已完成劳动任务，离开了监狱`;
  }

  async function checkJailStatus(ctx, config, session, targetUserId) {
    let user;
    if (targetUserId) {
      // Try to resolve target user
      user = await getUser6(ctx, targetUserId, session);
      if (typeof user === "string") {
        const resolved = await resolveTargetUser(ctx, session, targetUserId);
        if (resolved) {
          user = resolved;
        } else {
          return "❌ 找不到该用户";
        }
      }
    } else {
      user = await getUser6(ctx, session.userId, session);
      if (typeof user === "string") return user;
    }

    const isSelf = !targetUserId || user.userId === session.userId;
    const name = isSelf ? "你" : user.nickname;

    if (!user.isInJail) {
      return `✅ ${name}当前不在监狱中`;
    }
    const cooldown = resolveJailCooldown(config);
    const now = Date.now();
    const workStartTime = user.jailWorkStartTime || 0;

    let statusText = "🛑 未开始劳动";
    let timeText = "需要手动开始";

    if (workStartTime > 0) {
      const penalty = user.jailPenalty || 0;
      const effectiveCooldown = cooldown + penalty;
      if (now - workStartTime >= effectiveCooldown) {
        statusText = "✅ 劳动已完成";
        timeText = "可随时出狱";
      } else {
        const remaining = Math.ceil((effectiveCooldown - (now - workStartTime)) / 60000);
        statusText = "🔨 正在劳动中";
        timeText = `剩余 ${remaining} 分钟${penalty > 0 ? `（含惩罚 ${Math.ceil(penalty / 60000)} 分钟）` : ""}`;
      }
    }

    const bailAmount = calculateBailAmount(user, config);
    return `=== 监狱状态 (${user.nickname}) ===
💸 单次缝纫收益：100 - 150 金币
📊 当前状态：${statusText}
⏳ 时间进度：${timeText}
💵 交保费用：${bailAmount} 金币
💡 输入"踩缝纫机"开始或结算劳动，输入"交保出狱"立即离开
⚠️ 也可以尝试"越狱"，但失败会加刑，非必要不建议尝试！`;
  }

  async function bailOut(ctx, config, session) {
    let user = await getUser6(ctx, session.userId, session);
    if (typeof user === "string") return user;
    if (!user.isInJail) {
      return "✅ 你不在监狱中，无需交保。";
    }
    const bailAmount = calculateBailAmount(user, config);
    const wealthProtected = await isWealthProtected(ctx, session, user);
    let taxAmount = calculatePurchaseTax(config, bailAmount, user, { wealthProtected });
    let waiverTip = "";
    if (taxAmount > 0) {
      const waiver = await applyTaxWaiverHelper(ctx, session, user, taxAmount, { label: "保释税金" });
      taxAmount = waiver.amount;
      waiverTip = waiver.tip;
    }
    const totalCost = bailAmount + taxAmount;
    let autoWithdrawNotice = "";
    const cover = await ensureSufficientBalance(ctx, user, totalCost);
    user = cover.user;
    autoWithdrawNotice = cover.notice;
    if (user.balance < totalCost) {
      const notice = autoWithdrawNotice ? `\n${autoWithdrawNotice}` : "";
      return `❌ 交保需要${totalCost}金币，但你只有${user.balance}金币${notice}`;
    }
    const now = Date.now();
    const balanceAfter = user.balance - totalCost;
    await ctx.database.set("player_market_users", { userId: user.userId }, {
      balance: balanceAfter,
      isInJail: false,
      jailStartTime: 0,
      jailWorkStartTime: 0,
      jailReason: "",
      jailWorkIncome: 0,
      jailWorkCount: 0,
      lastJailWorkTime: now,
      lastJailVictimId: "",
    });
    invalidateWealthCache(session);
    await creditSystemAccount(ctx, totalCost);
    if (taxAmount > 0) {
      await ctx.taxService?.recordTax(session, taxAmount);
    }
    await transactionService?.logTransaction(ctx, { ...user, balance: balanceAfter }, {
      direction: "expense",
      category: transactionService?.categories.JAIL_BAIL,
      amount: totalCost,
      description: taxAmount > 0 ? "监狱交保出狱（含税）" : "监狱交保出狱",
      balanceAfter,
    });
    const notice = autoWithdrawNotice ? `\n${autoWithdrawNotice}` : "";
    return `✅ 交保成功！
💰 支付保释金：${bailAmount}金币${taxAmount > 0 ? `\n💸 税金：${taxAmount}金币` : ""}
🏁 你已离开监狱${notice}${waiverTip}`;
  }

  async function jailRoster(ctx, config, session) {
    const users = await ctx.database.get("player_market_users", { isInJail: true });
    if (!users.length) {
      return "监狱目前是空的";
    }
    const lines = users.map((user) => `${user.nickname} - 完成劳动后即可释放`);
    return `=== 监狱名单 ===\n${lines.join("\n")}`;
  }


  async function jailbreak(ctx, config, session) {
    let user = await getUser6(ctx, session.userId, session);
    if (typeof user === "string") return user;
    if (!user.isInJail) {
      return "✅ 你不在监狱中，无需越狱。";
    }

    const successRate = config.监狱系统?.越狱成功率 ?? 0.1;
    const penaltyMinutes = config.监狱系统?.越狱失败惩罚 ?? 10;
    const isSuccess = Math.random() < successRate;
    const now = Date.now();

    if (isSuccess) {
      await ctx.database.set("player_market_users", { userId: user.userId }, {
        isInJail: false,
        jailStartTime: 0,
        jailWorkStartTime: 0,
        jailReason: "",
        jailWorkIncome: 0,
        jailWorkCount: 0,
        lastJailWorkTime: now,
        lastJailVictimId: "",
        jailPenalty: 0
      });
      return `🏃‍♂️ 💨 越狱成功！你趁狱警不注意溜了出来，重获自由！`;
    } else {
      const currentPenalty = user.jailPenalty || 0;
      const cooldown = resolveJailCooldown(config);
      const required = cooldown + currentPenalty;
      const workStartTime = user.jailWorkStartTime || 0;
      let newStartTime = workStartTime;

      // If user has "banked" time (worked longer than required), clamp it so penalty bites
      if (workStartTime > 0) {
        const elapsed = now - workStartTime;
        if (elapsed > required) {
          const surplus = elapsed - required;
          newStartTime += surplus;
        }
      }

      const newPenalty = currentPenalty + (penaltyMinutes * 60 * 1000);

      await ctx.database.set("player_market_users", { userId: user.userId }, {
        jailPenalty: newPenalty,
        jailWorkStartTime: newStartTime
      });
      return `👮‍♂️ 越狱失败！被狱警抓个正着，刑期增加 ${penaltyMinutes} 分钟！\n乖乖去踩缝纫机吧。`;
    }
  }

  async function adminJail(ctx, config, session, targetUserId, reason) {
    if (!targetUserId) return "❌ 请指定目标用户";

    console.log(`[Jail Debug] targetUserId: ${targetUserId}, guildId: ${session.guildId}, channelId: ${session.channelId}`);

    // Try to get user, if not found, try to create one
    let user = await getUser6(ctx, targetUserId, session);

    if (typeof user === "string") {
      // Try resolving via resolveTargetUser (handles mentions, etc.)
      const resolved = await resolveTargetUser(ctx, session, targetUserId);
      if (resolved) {
        user = resolved;
      } else {
        // User not found or not registered. Try to auto-register if we can get info.
        try {
          const member = await session.bot.getGuildMember(session.guildId, targetUserId);
          const nickname = member.nick || member.user?.name || "未知用户";
          const result = await deps.ensurePlayerProfile(ctx, config, session, {
            targetUserId,
            nickname,
            manual: true // Mark as manually created (or auto-registered by admin)
          });
          if (result.user) {
            user = result.user;
          } else {
            return "❌ 无法创建目标用户数据。";
          }
        } catch (e) {
          console.error("[Jail Error] Auto-register failed:", e);
          return "❌ 找不到该用户，且无法获取其信息进行自动登记。";
        }
      }
    }

    if (user.isInJail) {
      return `⚠️ ${user.nickname} 已经在监狱里了`;
    }

    const now = Date.now();
    await ctx.database.set("player_market_users", { userId: user.userId }, {
      isInJail: true,
      jailStartTime: now,
      jailReason: reason || "管理员强制监禁",
      jailWorkStartTime: 0,
      jailWorkIncome: 0,
      jailWorkCount: 0,
      lastJailWorkTime: 0,
      jailPenalty: 0
    });

    return `👮‍♂️ 已将 ${user.nickname} 关进监狱！\n📝 罪名：${reason || "管理员强制监禁"}`;
  }

  async function adminRelease(ctx, config, session, targetUserId) {
    if (!targetUserId) return "❌ 请指定目标用户";

    // Try to get user
    let user = await getUser6(ctx, targetUserId, session);

    if (typeof user === "string") {
      // Try resolving via resolveTargetUser
      const resolved = await resolveTargetUser(ctx, session, targetUserId);
      if (resolved) {
        user = resolved;
      } else {
        return "❌ 找不到该用户";
      }
    }

    if (!user.isInJail) {
      return `⚠️ ${user.nickname} 不在监狱里`;
    }

    const now = Date.now();
    await ctx.database.set("player_market_users", { userId: user.userId }, {
      isInJail: false,
      jailStartTime: 0,
      jailWorkStartTime: 0,
      jailReason: "",
      jailWorkIncome: 0,
      jailWorkCount: 0,
      lastJailWorkTime: now,
      lastJailVictimId: "",
      jailPenalty: 0
    });

    return `👮‍♂️ 已将 ${user.nickname} 释放出狱！`;
  }

  function registerJailCommands(ctx, config) {
    const slaveCommand = ctx.command("大牛马时代");
    slaveCommand
      .subcommand("踩缝纫机", "在监狱里踩缝纫机赚取收入")
      .alias("监狱打工")
      .action(async ({ session }) => {
        const respond = setupMessageRecall(session, ctx, config, "general");
        const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
        if (taxCheck) return await respond(taxCheck);
        return await respond(await jailWork(ctx, config, session));
      });
    slaveCommand
      .subcommand("交保出狱", "支付保释金立即离开监狱")
      .alias("交保")
      .alias("保释")
      .action(async ({ session }) => {
        const respond = setupMessageRecall(session, ctx, config, "general");
        const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
        if (taxCheck) return await respond(taxCheck);
        return await respond(await bailOut(ctx, config, session));
      });
    slaveCommand.subcommand("监狱状态 [target:user]", "查看监狱状态").action(async ({ session }, target) => {
      const respond = setupMessageRecall(session, ctx, config, "general");
      const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
      if (taxCheck) return await respond(taxCheck);
      return await respond(await checkJailStatus(ctx, config, session, target));
    });
    slaveCommand.subcommand("监狱名单", "查看当前在监狱中的玩家列表").action(async ({ session }) => {
      const respond = setupMessageRecall(session, ctx, config, "general");
      const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
      if (taxCheck) return await respond(taxCheck);
      return await respond(await jailRoster(ctx, config, session));
    });

    slaveCommand.subcommand("越狱", "尝试越狱（有风险）").action(async ({ session }) => {
      const respond = setupMessageRecall(session, ctx, config, "general");
      const taxCheck = await checkTaxBeforeCommand(ctx, config, session);
      if (taxCheck) return await respond(taxCheck);
      return await respond(await jailbreak(ctx, config, session));
    });

    slaveCommand.subcommand("坐牢 <target:user> [reason:text]", "管理员强制用户入狱", { authority: 3 })
      .action(async ({ session }, target, reason) => {
        const respond = setupMessageRecall(session, ctx, config, "general");
        return await respond(await adminJail(ctx, config, session, target, reason));
      });

    slaveCommand.subcommand("出狱 <target:user>", "管理员强制释放用户", { authority: 3 })
      .action(async ({ session }, target) => {
        const respond = setupMessageRecall(session, ctx, config, "general");
        return await respond(await adminRelease(ctx, config, session, target));
      });
  }

  return { registerJailCommands };
}

module.exports = { createJailModule };

