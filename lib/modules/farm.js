...
async function renderCropGallery(ctx, list = crops) {
  if (ctx?.puppeteer) {
    let page;
    try {
      page = await ctx.puppeteer.page();
      const html = buildCropGalleryHtml(list);
      await page.setContent(html, { waitUntil: "networkidle0" });
      await page.waitForSelector(".gallery-container", { timeout: 2e3 });
      const container = await page.$(".gallery-container");
      if (container) {
      ...
      ctx.logger?.warn?.(`[slave-market] crop gallery render failed: ${error.message}`);
    } finally {
      if (page) {
        try {
          await page.close();
  ...
  return buildCropGalleryText(list);
}
...
function createFarmModule(deps) {
  ...
  async function plantCrop(ctx, config, session, cropName) {
    ...
    if (!cropName) {
      return await renderCropGallery(ctx);
    }
    ...
    if (!crop) {
      const gallery = await renderCropGallery(ctx);
      if (typeof gallery === "string") {
        return `找不到作物"${cropName}"
${gallery}`;
      }
      return `找不到作物"${cropName}"。发送\`种地\`查看可用作物列表。`;
    }
    const privileged = isAdmin(ctx, config, user.userId, session);
    const wealthProtected = await isWealthProtected(ctx, session, user);
    const purchaseTax = privileged ? 0 : calculatePurchaseTax(config, crop.price, user, { wealthProtected });
