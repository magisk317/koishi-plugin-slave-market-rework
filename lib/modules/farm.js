...
function buildCropGalleryText(list = crops) {
  const labelWidth = Math.max(...list.map((crop) => Array.from(formatCropLabel(crop)).length));
  const formatColumn = (text, targetLength) => {
    const value = String(text ?? "");
    const padLength = Math.max(0, targetLength - Array.from(value).length);
    return value + " ".repeat(padLength);
  };
  const rows = list.map((crop) => {
    const label = formatCropLabel(crop);
    const labelPadding = " ".repeat(Math.max(0, labelWidth - Array.from(label).length));
    const cost = formatColumn(crop.price, 4);
    const growth = formatColumn(formatGrowthTime(crop.growthTime), 4);
    const harvest = formatColumn(crop.harvestPrice, 4);
    return `${label}${labelPadding}|${cost}|${growth}|${harvest}`;
  });
  const tableText = `🖼️ 作物图鉴
[作物|成本|成长|产值]
${rows.join("\n")}`;
  const descText = `📋 描述：
- ${list.map((crop) => `${formatCropLabel(crop)}：${crop.description}`).join("\n- ")}
（发送“种地 作物名”即可种植）`;
  return {
    fullText: `${tableText}
${descText}`,
...
function createFarmModule(deps) {
  ...
  async function plantCrop(ctx, config, session, cropName) {
    ...
    if (!cropName) {
      return await renderCropGallery(ctx, session);
    }
    const crop = crops.find((c) => c.name === cropName);
    if (!crop) {
      return `找不到作物"${cropName}"。发送“种地”获取最新作物图鉴。`;
    }
    const now = Date.now();
    if (user.currentCrop) {
