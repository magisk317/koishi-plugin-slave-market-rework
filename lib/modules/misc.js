...
const { getResetRecord, upsertResetRecord, isResetBanned, RESET_LIMIT } = require("../utils/resetLimit");
...
可以随时重新打工/注册，重新开始。`;
  ...
  function registerMiscCommands(ctx, config) {
    ...
    restartCommand.action(async ({ session }) => {
      ...
      const now = Date.now();
      const resetRecord = await getResetRecord(ctx, session.userId);
      if (await isResetBanned(ctx, session.userId || player.plainUserId)) {
        return await respond("🚫 由于多次重开违规，你已被永久封禁，无法再使用本功能。");
      }
      const resetsUsed = resetRecord ? (resetRecord.resetCount ?? (resetRecord.lastResetTime ? 1 : 0)) : 0;
      const lastResetTime = resetRecord?.lastResetTime || 0;
      ...
      if (!pendingAt || now - pendingAt > RESET_CONFIRM_TIMEOUT) {
        ...
        if (resetsUsed >= RESET_LIMIT) {
          return await respond("⚠️ 你已超过允许的重开次数，本次操作完成后账号将被永久封禁。若仍要继续，请再次发送“重开”确认。");
        }
      ...
      const newCount = resetsUsed + 1;
      const banApplied = newCount > RESET_LIMIT;
      await upsertResetRecord(ctx, {
        userId: player.userId,
        scopeId: player.scopeId,
        plainUserId: player.plainUserId || session.userId
      }, {
      ...
      if (banApplied) {
        return await respond(`${result}\n🚫 你已超过允许的重开次数，现已被永久封禁，所有命令将不再响应。`);
      }
      if (newCount >= RESET_LIMIT) {
        return await respond(`${result}\n⚠️ 你已达到重开次数上限，再次重开将导致永久封禁。`);
      }
      return await respond(result);
    });
