...
const { resolveScopeInput, extractMentionedUserId } = require("../utils/playerHelpers");
...
function createAdminModule(deps) {
  ...
  function registerAdminCommands(ctx, config) {
    ...
    slaveCommand.subcommand("拉黑 <target:string>", "拉黑指定用户（管理员）").action(async ({ session }, target) => {
      ...
      const targetId = ((target || "").trim() || mentionId || "").trim();
      if (!targetId) return await respond("❌ 请输入要拉黑的用户或直接@他");
      const scopeId = session.guildId || session.channelId || "";
      const orList = [{ userId: targetId }];
      if (scopeId && !targetId.includes("#")) {
        orList.push({ userId: `${scopeId}#${targetId}` });
      } else if (targetId.includes("#")) {
        orList.push({ plainUserId: targetId.split("#").pop() });
      }
      if (/^\d+$/.test(targetId)) {
        orList.push({ plainUserId: targetId });
      }
      const [user] = await ctx.database.get("player_market_users", { $or: orList });
      ...
      if (!config.用户控制) config.用户控制 = { 白名单: [], 黑名单: [] };
      if (!Array.isArray(config.用户控制.黑名单)) config.用户控制.黑名单 = [];
      if (user.plainUserId && !config.用户控制.黑名单.includes(user.plainUserId)) {
        config.用户控制.黑名单.push(user.plainUserId);
      }
