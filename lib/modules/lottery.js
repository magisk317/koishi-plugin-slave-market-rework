...
const DEFAULT_LOTTERY_OPTIONS = [
  { id: "mini", name: "迷你券", price: 500, reward: 2000 },
  { id: "mega", name: "进阶券", price: 1000, reward: 5000 },
  { id: "ultra", name: "豪华券", price: 2500, reward: 15000 }
];
...
function normalizeOptions(config) {
  ...
  return list.map((item, index) => {
    const priceValue = Number(item?.price ?? item?.价格 ?? DEFAULT_LOTTERY_OPTIONS[index]?.price ?? 0);
    const rewardValue = Number(item?.reward ?? item?.奖金 ?? DEFAULT_LOTTERY_OPTIONS[index]?.reward ?? priceValue * 4);
...
function getLotterySettings(config) {
  const root = config?.彩票 || {};
  const dailyLimitValue = Number(root.每日限购 ?? root.dailyLimit);
  return {
    dailyLimitPerOption: Number.isFinite(dailyLimitValue) && dailyLimitValue > 0 ? Math.floor(dailyLimitValue) : DEFAULT_LIMIT,
    winRate: clampRate(Number(root.中奖率 ?? root.winRate)),
...
function createLotteryModule(deps) {
  ...
  async function purchaseTicket(ctx, config, session, optionKey) {
    ...
    if (bought >= settings.dailyLimitPerOption) {
      return `⛔ ${option.name} 今日限购 ${settings.dailyLimitPerOption} 张，明天再来试手气吧。`;
    }
  ...
  async function renderLotteryInfo(ctx, config, session) {
    ...
    const infoLines = await Promise.all(settings.options.map(async (item) => {
      ...
      const remaining = Math.max(0, settings.dailyLimitPerOption - bought);
      const desc = item.description ? `｜${item.description}` : "";
      return `• ${item.name}（${item.id}）｜售价 ${item.price}｜奖金 ${item.reward}${desc}｜今日剩余 ${remaining}/${settings.dailyLimitPerOption}`;
    }));
    return `=== 幸运彩券 ===
🎯 中奖率：${(settings.winRate * 100).toFixed(0)}%
📅 每种每日限购：${settings.dailyLimitPerOption} 张
...
${recent.join("\n")}`;
  ...
  function registerLotteryCommands(ctx, config) {
    const command = ctx.command("大牛马时代.彩票", "娱乐性彩票玩法");
