...
function createWebUI(ctx, config, deps) {
    ...
    ctx.server.post('/slave-market/api/users/:userId/plant', async (koaCtx) => {
        try {
            const { userId } = koaCtx.params;
            const { cropName } = koaCtx.request.body;

            if (!plant) {
                throw new Error('Plant function not available');
            }
            ...
            // plant returns a string or object. If it's an error string, we might want to parse it?
            // But usually it returns a success message string.
    ...
    ctx.server.post('/slave-market/api/users/:userId/harvest', async (koaCtx) => {
        try {
            ...
            if (!harvest) {
                throw new Error('Harvest function not available');
            }
    ...
    // API: Get Single User
    ctx.server.get('/slave-market/api/users/:userId', async (koaCtx) => {
        try {
            const { userId } = koaCtx.params;
            const user = (await ctx.database.get('player_market_users', { userId }))[0];
            ...
            koaCtx.body = serializeBigInt({ success: true, data: user });
        } catch (err) {
